...
const OlcekOlustur = ({ onBack, onSave, editingScale }) => {
  ...
  // Düzenleme modunda form verilerini yükle
  useEffect(() => {
    if (editingScale) {
      setScaleName(editingScale.name || '');
      setScaleDescription(editingScale.description || '');
      setScaleType(editingScale.scaleType || 'basic');
      setCutoffScore(editingScale.cutoffScore || '');
      setSubscales(editingScale.subscales || []);
      setQuestions(editingScale.questions || []);
      setScoringRules(editingScale.scoringRules || {
